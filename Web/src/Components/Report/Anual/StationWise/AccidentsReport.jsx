...
const AccidentsReport = () => {
  ...
  useEffect(() => {
    ...
    if (start && end && type && stationParam) {
      setStartYear(start);
      setEndYear(end);
      setReportType(type);
      ...
      if (type === 'accidents') {
        fetchAccidentsData(start, end, stationParam);
      } else if (type === 'casualties') {
        fetchCasualtiesData(start, end, stationParam);
      } else if (type === 'reasons') {
        fetchReasonsData(start, end, stationParam);
      }
    }
  }, [location]);

  const fetchAccidentsData = async (start, end, station) => {
    setLoading(true);
  ...
  const fetchCasualtiesData = async (start, end, station) => {
    setLoading(true);
    const accidentsRef = collection(db, 'accident_report');
    const reportData = {};
  ...
  const fetchReasonsData = async (start, end, station) => {
    setLoading(true);
    const accidentsRef = collection(db, 'accident_report');
    const reportData = {};
