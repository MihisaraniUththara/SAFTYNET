...
const AccidentDetailsModalReject = ({ report, getStatus, getStatus1, onClose, A8F, A9F, getUrban, A20F, A21F, A22F, A23F, A24F, A25F, A26F, A27F, A30F }) => {
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-5 rounded-lg w-1/3">
        <h2 className="text-xl font-bold mb-4"><center>Accident Report Details</center></h2>
        <div>
          <div className='text-right gap-3 flex-row justify-between'>
            <div>{getStatus(report.submit, report.reject, report.rejectH)}</div>
            <div>{getStatus1(report.submit, report.oicApp, report.headApp)}</div></div>
...
export default AccidentDetailsModalReject;
